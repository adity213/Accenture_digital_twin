...
## 12. Synthetic Data Realism Upgrade Log (Phases 17–25)

| Phase | Core Mechanism | Grounded Physics & Math Formulation | Empirical Verification Metric | Source Implementation |
| :--- | :--- | :--- | :--- | :--- |
| **Phase 17** | **Latent AR(1) Load State** | Continuous autoregressive latent state: $\text{load}_t = \rho \cdot \text{load}_{t-1} + (1-\rho)\mu_{\text{target}} + \epsilon_t$ ($\rho = 0.90$) driving physical telemetry (vibration, temperature, power, takt). | $\text{Lag-1 Autocorr} = 0.63$, $\text{Corr}(\text{Vib}, \text{Temp}) = +0.65$, $\text{Corr}(\text{CT}, \text{Vib}) = +0.33$. | [`generator.py:93`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/simulator/generator.py#L93) |
| **Phase 18** | **Lognormal Cycle Times** | Right-skewed lognormal machining dwell times: $\text{CT} \sim \text{Lognormal}(\mu, \sigma)$ coupled to $\text{load\_state}$ via $\mu = \ln(\mu_{\text{target}} \cdot \text{mult}) - \frac{\sigma^2}{2}$. | Skewness $> +0.47$ on manual lines; positive tail $> 1.30\times$ takt without artificial hard clipping. | [`generator.py:22`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/simulator/generator.py#L22) |
| **Phase 19** | **Category-Differentiated CV & Multipliers** | Process-stratified parameterization:<br>1. *Automated Precision* ($\text{CV}=0.040, 0.6\times$ defects)<br>2. *Automated Process* ($\text{CV}=0.060, 1.0\times$ defects)<br>3. *Manual Operations* ($\text{CV}=0.130, 2.8\times$ defects). | Precision $\text{CV}=0.0413$, Process $\text{CV}=0.0615$, Manual $\text{CV}=0.1511$ (under shift fatigue); natural defect rates $0.48\%, 0.80\%, 2.24\%$. | [`generator.py:33`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/simulator/generator.py#L33) |
| **Phase 20** | **Decoupled Anomaly Signatures** | Anomaly-specific physical decoupling:<br>- `gradual_drift`: vibration + thermal runaway, active dwell intact.<br>- `sudden_stoppage`: zero active power, zero mechanical vibration, infinite cycle time.<br>- `latent_defect`: invisible upstream, detected at downstream QC.<br>- `energy_waste`: unthrottled idle power draw without cycle delay. | Zero feature cross-leakage; independent physical sensor channels validated across 5 distinct fault modes. | [`generator.py:310`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/simulator/generator.py#L310) |
| **Phase 21** | **Emergent Tool Wear & Control Group** | Continuous tool wear accumulation ($\dot{w} = \alpha_{\text{cat}} \cdot (1 + \text{load}) + \text{shocks}$), stochastic unscheduled breakdown trigger ($w > 0.85$), periodic maintenance resets, zero-drift control stations (`ST03`, `ST15`, `ST31`). | 1,559 organic failure triggers over 8,500 ticks; Category-balanced Control FP rate ($28.69\%$) $\le$ Drifting ($28.98\%$); 0 hidden variables in feature vector. | [`generator.py:96`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/simulator/generator.py#L96) |
| **Phase 22** | **SPC Baseline Recalibration** | Category-differentiated Shewhart/EWMA variance baseline: $\sigma_{\text{base}} = \max(0.5\text{s}, T_{\text{target}} \cdot \text{CV}_{\text{cat}})$. | Nominal false alarm rate ($|z| > 3.0$) bounded at $0.00\%-1.16\%$; cross-category discrepancy $\le 1.16\%$. | [`spc.py:45`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/pipeline/spc.py#L45) |
| **Phase 23** | **Retrained Model & 7-Regime OOD Suite** | Multi-seed 800,000-row retraining on physics-upgraded generator; GBDT evaluation across 7 operational regimes (Spatial, Symptom, Speed Stress, Severity, Dropout, Emergent Wear). | Held-out Test: Bottleneck ROC-AUC $0.977$, PR-AUC $0.946$, Recall $95.1\%$; Defect ROC-AUC $0.719$, PR-AUC $0.344$; Baseline I.I.D. ROC-AUC $0.979$ / $0.776$. | [`train_risk_model.py:1`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/scripts/train_risk_model.py#L1) |
| **Phase 24** | **3-Shift Circadian Productivity & Fatigue** | 1440-tick 3-shift circadian schedule (Day/Evening/Night) with non-linear intra-shift human fatigue curve ($\Delta \tau = 0.05(\tau - 0.4\sin(2\pi\tau))$). | Night shift defect rate $+39.84\%$ on manual stations; cycle time $+12.34\%$; sinusoidal diurnal encodings $\sin(\frac{2\pi t}{1440}), \cos(\frac{2\pi t}{1440})$. | [`generator.py:157`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/simulator/generator.py#L157) |
| **Phase 25** | **Residual Learning & Shadow Mode Router** | Deterministic physics baseline heuristic running in shadow mode; divergence routing ($\Delta_{\text{div}} > 0.45$) and sensor confidence fail-safe fallback ($c < 0.65$). | Mean routing + GBDT inference latency: $6.01\text{ ms / sample}$ ($6,012.3\ \mu\text{s}$); deterministic physics fallback activated on sensor blackout / high divergence. | [`risk_model.py:301`](file:///c:/Android%20Projects/accenture/digitaltwin-ai/pipeline/risk_model.py#L301) |
