...
Systematically evaluates model generalization across 7 industrial operating regimes:
...
Outputs:
- Rich comparative summary table to stdout (separated into Bottleneck and Defect)
...
def evaluate_regime(
    ...
    bn_metrics = model._evaluate(model.bottleneck_model, X_test, y_bn_test, threshold=threshold)
    def_metrics = model._evaluate(model.defect_model, X_test, y_def_test, threshold=threshold)
    
    total_count = len(y_bn_test)
    ...
    return {
        ...
        "bottleneck": {
            "positives": sum(y_bn_test),
            "positive_rate": round(sum(y_bn_test) / max(1, total_count), 4),
            "roc_auc": bn_metrics["auc"],
            "pr_auc": bn_metrics["pr_auc"],
            "precision": bn_metrics["precision"],
            "recall": bn_metrics["recall"],
            "f1": bn_metrics["f1"],
            "brier_score": bn_metrics["brier_score"],
            "false_alarm_rate": bn_metrics["false_alarm_rate"],
        },
        "defect": {
            "positives": sum(y_def_test),
            "positive_rate": round(sum(y_def_test) / max(1, total_count), 4),
            "roc_auc": def_metrics["auc"],
            "pr_auc": def_metrics["pr_auc"],
            "precision": def_metrics["precision"],
            "recall": def_metrics["recall"],
            "f1": def_metrics["f1"],
            "brier_score": def_metrics["brier_score"],
            "false_alarm_rate": def_metrics["false_alarm_rate"],
        }
    }
...
def main():
    ...
    # ---------------------------------------------------------------------------------
    print("[1/7] Evaluating Baseline I.I.D. (Within-Distribution Split)...")
    ...
    y_def_iid_train, y_def_iid_test = y_def_iid[:split_idx], y_def_iid[split_idx:]
    ...
    res_iid = evaluate_regime(
        model_iid, X_iid_test, y_bn_iid_test, y_def_iid_test,
    ...
    # ---------------------------------------------------------------------------------
    print("[2/7] Evaluating Spatial OOD (Cross-Station: Train ST01-ST30 -> Test ST31-ST40)...")
    ...
    X_sp_test, y_bn_sp_test, y_def_sp_test = parse_features_and_labels(spatial_test_rows)
    ...
    res_spatial = evaluate_regime(
        model_spatial, X_sp_test, y_bn_sp_test, y_def_sp_test,
    ...
    # ---------------------------------------------------------------------------------
    print("[3/7] Evaluating Phenomenological OOD (Train Isolated -> Test Compound Multi-Faults)...")
    ...
    X_comp_test, y_bn_comp_test, y_def_comp_test = parse_features_and_labels(compound_test_rows)
    ...
    res_compound = evaluate_regime(
        model_compound, X_comp_test, y_bn_comp_test, y_def_comp_test,
    ...
    # ---------------------------------------------------------------------------------
    print("[4/7] Evaluating Operational Speed Stress (+20% Takt Acceleration)...")
    speed_test_rows = load_dataset_rows(data_dir / "test_speed_stress.csv")
    X_speed_test, y_bn_speed_test, y_def_speed_test = parse_features_and_labels(speed_test_rows)

    res_speed = evaluate_regime(
        model_iid, X_speed_test, y_bn_speed_test, y_def_speed_test,
    ...
    # ---------------------------------------------------------------------------------
    print("[5/7] Evaluating Severity Stress (Extreme Damage Out-of-Bounds)...")
    sev_test_rows = load_dataset_rows(data_dir / "test_severity_stress.csv")
    X_sev_test, y_bn_sev_test, y_def_sev_test = parse_features_and_labels(sev_test_rows)

    res_severity = evaluate_regime(
        model_iid, X_sev_test, y_bn_sev_test, y_def_sev_test,
    ...
    X_sensor_test, y_bn_sensor_test, y_def_sensor_test = parse_features_and_labels(sensor_test_rows)

    res_sensor = evaluate_regime(
        model_iid, X_sensor_test, y_bn_sensor_test, y_def_sensor_test,
    ...
    if emergent_test_file.exists():
        ...
        X_emergent_test, y_bn_emergent_test, y_def_emergent_test = parse_features_and_labels(emergent_test_rows)

        res_emergent = evaluate_regime(
            model_iid, X_emergent_test, y_bn_emergent_test, y_def_emergent_test,
    ...
    # ---------------------------------------------------------------------------------
    header_fmt = "{:<33s} | {:>14s} | {:>14s} | {:>14s} | {:>14s}"
    row_fmt    = "{:<33s} | {:>6.3f} / {:>5.3f} | {:>6.3f} / {:>5.3f} | {:>5.1f}% / {:>4.1f}% | {:>5.1f}% / {:>4.1f}%"

    print("\n" + "=" * 115)
    print(" COMPREHENSIVE OUT-OF-DISTRIBUTION (OOD) GENERALIZATION BENCHMARK (Bottleneck / Defect)")
    print("=" * 115)
    ...
    base_bn_auc = res_iid["bottleneck"]["roc_auc"]
    base_def_auc = res_iid["defect"]["roc_auc"]

    for r in results:
        print(row_fmt.format(
            r["regime"],
            r["bottleneck"]["roc_auc"], r["defect"]["roc_auc"],
            r["bottleneck"]["pr_auc"], r["defect"]["pr_auc"],
            r["bottleneck"]["recall"] * 100, r["defect"]["recall"] * 100,
            r["bottleneck"]["false_alarm_rate"] * 100, r["defect"]["false_alarm_rate"] * 100,
        ))

    print("-" * 115)
    print(" Generalization Gaps (Relative to Baseline I.I.D. ROC-AUC):")
    for r in results[1:]:
        delta_bn_auc = r["bottleneck"]["roc_auc"] - base_bn_auc
        delta_def_auc = r["defect"]["roc_auc"] - base_def_auc
        sign_bn_auc = "+" if delta_bn_auc >= 0 else ""
        sign_def_auc = "+" if delta_def_auc >= 0 else ""
        print(f"   * {r['regime']:<32s}: BN dROC-AUC = {sign_bn_auc}{delta_bn_auc:.3f} | Def dROC-AUC = {sign_def_auc}{delta_def_auc:.3f}")
    print("=" * 115)
    ...
    summary_payload = {
        ...
        "decision_threshold": args.threshold,
        "baseline_bn_auc": base_bn_auc,
        "baseline_def_auc": base_def_auc,
        "regimes": results
    }
